...
import {DocumentCenter,explainLocalized} from './main';
...
const template:TemplateSummary={schema_version:'enotario.document-template/v2',id:'ma.inheritance',
  version:'1.4.0',slug:'herencia',title_es:'Acta de herencia',title_fr:'Acte d’héritage',title_ar:'إراثة',
  description_es:'Piloto',description_fr:'Pilote',language:'ar-MA',roles:[]};
...
it('localizes native save, LAN and receipt errors without exposing backend text',()=>{
  expect(explainLocalized('DOCX_WRITE_FAILED','fr')).toContain('document Word');
  expect(explainLocalized('DOCX_WRITE_FAILED','ar')).toContain('Word');
  expect(explainLocalized('LAN_IP_INVALID','fr')).toContain('IPv4');
  expect(explainLocalized('SAVE_RECEIPT_READ_FAILED','ar')).toContain('إيصال');
});
...
async function fixture({requests=[item]}:{requests?:DocumentGenerationRequest[]}={}){
  ...
  if(requests.length){
    await screen.findByText('Acte d’héritage');
    await waitFor(()=>expect(screen.getByRole('button',{name:'Enregistrer et ouvrir dans Word'}).hasAttribute('disabled')).toBe(false));
  }
...
describe('partial Word protected saving',()=>{
  it('keeps new requests disabled during the finishing-only window',async()=>{
    const api={templates:vi.fn().mockResolvedValue([template])} as unknown as CaptureApi;
    const client=new QueryClient({defaultOptions:{queries:{retry:false}}});
    render(<AppTheme><QueryClientProvider client={client}><DocumentCenter api={api} identities={[]}
      requests={[]} onChanged={()=>{}} active newWorkAllowed={false}/>
    </QueryClientProvider></AppTheme>);
    expect((await screen.findByRole('button',{name:'Envoyer à la file'})).hasAttribute('disabled')).toBe(true);
    expect(screen.getByText(/seulement de terminer les demandes existantes/)).toBeTruthy();
  });
  ...
  it('keeps the request when native saving is cancelled',async()=>{
    ...
    await screen.findByText('Enregistrement annulé. La demande reste dans la file.');
  ...
  it('retires exactly the generated revision only after saving and then acknowledges the receipt',async()=>{
    ...
    fireEvent.click(screen.getByRole('button',{name:'Enregistrer et ouvrir dans Word'}));
    await screen.findByText('Document enregistré et ouvert dans Word.');
  ...
  it('offers Explorer when the associated application cannot open the saved Word',async()=>{
    ...
    await fixture();
    fireEvent.click(screen.getByRole('button',{name:'Enregistrer et ouvrir dans Word'}));
    await screen.findByText('Le document a été enregistré, mais Windows n’a pas pu ouvrir l’application associée.');
  ...
  it('retries server closure with the same persisted key without generating or saving twice',async()=>{
    ...
    fireEvent.click(screen.getByRole('button',{name:'Enregistrer et ouvrir dans Word'}));
    const retry=await screen.findByRole('button',{name:'Retirer la demande enregistrée'});
    expect(screen.getByRole('button',{name:'Enregistrer et ouvrir dans Word'}).hasAttribute('disabled')).toBe(true);
    expect(screen.getByRole('button',{name:'Vérifier les attributions'}).hasAttribute('disabled')).toBe(true);
    await waitFor(()=>expect(onNavigationBlockedChange).toHaveBeenLastCalledWith(true));
    const unload=new Event('beforeunload',{cancelable:true});window.dispatchEvent(unload);
    expect(unload.defaultPrevented).toBe(true);
    fireEvent.click(retry);
    await screen.findByText('Document enregistré et demande retirée.');
  ...
  it('recovers after restart even when the live inbox is empty',async()=>{
    ...
    fireEvent.click(await screen.findByRole('button',{name:'Retirer la demande enregistrée'}));
    await screen.findByText('Document enregistré et demande retirée.');
    expect(completeDocumentRequest).toHaveBeenCalledWith(item.id,4,receipt.id);
    expect(native.acknowledge).toHaveBeenCalledWith(receipt.id);
  ...
  it('never recovers an unconfirmed file or a receipt from the complete-case domain',async()=>{
    ...
    const recover=await screen.findByRole('button',{name:'Récupérer'});
    expect(recover.hasAttribute('disabled')).toBe(true);
    expect(screen.getAllByRole('button',{name:'Récupérer'})).toHaveLength(1);
    expect(completeDocumentRequest).not.toHaveBeenCalled();
  });
  ...
  it('keeps a stale receipt and the independent file when the request revision changed',async()=>{
    ...
    fireEvent.click(await screen.findByRole('button',{name:'Récupérer'}));
    fireEvent.click(await screen.findByRole('button',{name:'Retirer la demande enregistrée'}));
    await screen.findByText(/Le fichier reste enregistré/);
    expect(native.acknowledge).not.toHaveBeenCalled();
    expect(screen.getByRole('button',{name:'Retirer la demande enregistrée'}).hasAttribute('disabled')).toBe(false);
  });
